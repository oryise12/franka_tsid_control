    ...
    def run_simulation(self):
        ...
        with mujoco.viewer.launch_passive(self.mj_model, self.mj_data) as viewer:
            ...
            # --- HQP 제어 로직 ---
                ...
                # ---------------------------------------------------------
                # 🛡️ [NEW] 관절 한계 회피 (Joint Limit Avoidance) 제약 조건
                # 관절 한계에 가까워지면 투명한 벽(쿠션)이 밀어내도록 가속도 상/하한을 만듭니다.
                # ---------------------------------------------------------
                margin = 0.05 # 한계치 0.05 rad (약 2.8도) 전부터 멈추도록 여유를 줌
                K_lim = 50.0
                D_lim = 2.0 * np.sqrt(K_lim)
                
                # 관절이 한계에 다가갈수록 허용되는 최대/최소 가속도를 제한
                ddq_ub = K_lim * (q_upper - margin - q_arm) - D_lim * v_arm
                ddq_lb = K_lim * (q_lower + margin - q_arm) - D_lim * v_arm
                
                # 너무 큰 숫자가 들어가는 것을 방지하고, lb가 ub보다 커지는 역전 현상 방지
                ...
                # 여기서 솔버에 lb(하한), ub(상한) 파라미터를 추가로 넘겨줍니다!
                ddq_1 = solve_qp(P1, q1, G_ineq, h_ineq, lb=ddq_lb, ub=ddq_ub, solver="osqp")
                ddq_final = None
                ...
                if ddq_1 is not None:
                    ...
                    # 2단계 솔버에도 동일하게 lb, ub 한계를 줍니다.
                    ddq_2 = solve_qp(P2, q2, G_ineq, h_ineq, A=A_eq, b=b_eq, lb=ddq_lb, ub=ddq_ub, solver="osqp")
